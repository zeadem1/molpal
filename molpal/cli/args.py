...
def add_acquisition_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        ...
        "--alpha",
        default="random",
        choices={"random", "greedy", "threshold", "ts", "ucb", "ei", "pi", "thompson", "md_ei"},
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="the alpha value for MD-EI metric (balance between exploitation and exploration). Higher values (e.g., 0.9) favor exploitation (EI), lower values (e.g., 0.5) favor exploration (diversity). Default: 0.8",
    )
    parser.add_argument(
        "--threshold",
        type=float,
...
def add_model_args(parser: ArgumentParser):
    ...
    model_parser.add_argument(
        "--model", choices=("rf", "gp", "nn", "mpn", "dmpn"), default="rf", help="the model type to use"
    )
    model_parser.add_argument(
        "--test-batch-size",
        type=int,
    ...
    model_parser.add_argument(
        "--message-passing",
        choices=("auto", "directed", "undirected"),
        default="auto",
        help=(
            "Message passing mode for MPN/DMPN models. "
            '"auto" keeps model defaults (mpn: directed-style updates, dmpn: directed). '
            '"directed" forces directed updates, "undirected" enables reverse-edge symmetrization.'
        ),
    )
...
def add_stopping_args(parser: ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="the minimum number of completed exploration rounds before internal convergence-based stopping can trigger",
    )
    parser.add_argument(
        "--stop-patience",
        type=int,
        default=2,
        help="the number of consecutive plateau checks required before internal convergence-based stopping can trigger",
    )
...
def clean_and_fix_args(args: Namespace):
    ...
    if args.metric != "ei" and args.metric != "pi":
    ...
    if args.model != "nn":
        args_to_remove |= set()
    if args.model != "mpn" and args.model != "dmpn":
        args_to_remove |= {"init_lr", "max_lr", "final_lr", "ddp", "precision", "directed", "undirected"}
    if args.model != "nn" and args.model != "mpn" and args.model != "dmpn":
        args_to_remove |= {"conf_method"}

    if not args.prune:
        args_to_remove |= {"prune_min_hit_prob"}

    # Resolve the user-facing message passing mode into explicit encoder flags.
    if args.model in {"mpn", "dmpn"}:
        if args.message_passing == "auto":
            args.directed = args.model == "dmpn"
            args.undirected = False
        elif args.message_passing == "directed":
            args.directed = True
            args.undirected = False
        elif args.message_passing == "undirected":
            args.directed = False
            args.undirected = True
        else:
            raise ValueError(f'Unrecognized message passing mode: "{args.message_passing}"')

    args_to_remove.add("message_passing")
