...
import ray
# Lazy import RayTrainer only when needed (for ddp mode)
...
def pop_mpnn_config(kwargs):
    return {key: kwargs.pop(key) for key in list(kwargs) if key in MPNN_CONFIG_KEYS}


def apply_message_passing_defaults(
    mpnn_config: Dict[str, object], *, directed_default: bool
) -> Dict[str, object]:
    config = dict(mpnn_config)
    config.setdefault("directed", directed_default)
    config.setdefault("undirected", False)

    if config["directed"] and config["undirected"]:
        raise ValueError('MPNN config cannot set both "directed" and "undirected" to True.')
...
class MPNN:
    ...
    def __init__(
        ...
        try:
            ...
            # Ray not initialized, check for GPU using torch
        ...
        if ngpu > 0:
            ...
            try:
                ...
                # Ray not initialized, use ncpu
                self.num_workers = ncpu
...
class MPNModel(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mpnn_config = apply_message_passing_defaults(
            pop_mpnn_config(kwargs), directed_default=False
        )

        self.build_model = partial(
            MPNN, ncpu=ncpu, ddp=ddp, precision=precision, model_seed=model_seed, **mpnn_config
        )
...
# D-MPNN Models (Directed Message Passing Neural Networks)
# These models use directed message passing for better molecular representation


class DMPNModel(Model):
    """Directed message-passing model that learns feature representations of inputs
    using directed message passing (D-MPNN) and passes these inputs to a feed-forward
    neural network to predict means.

    D-MPNN improves upon standard MPNN by using directed edges in the molecular graph,
    which better captures the directional nature of chemical bonds.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        test_batch_size = test_batch_size or 1000000
        mpnn_config = apply_message_passing_defaults(
            pop_mpnn_config(kwargs), directed_default=True
        )
    ...
    def get_means_and_vars(self, xs: List) -> NoReturn:
        raise TypeError("DMPNModel cannot predict variance!")
...
class DMPNDropoutModel(Model):
    """Directed message-passing network model that predicts means and variances through
    stochastic dropout during model inference.

    Combines D-MPNN architecture with dropout-based uncertainty estimation.
    """
...
class DMPNTwoOutputModel(Model):
    """Directed message-passing network model that predicts means and variances through
    mean-variance estimation (MVE).

    Combines D-MPNN architecture with MVE-based uncertainty estimation.
    """
