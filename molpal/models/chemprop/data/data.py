...
class MoleculeDatapoint:
    ...
    def mol(self) -> List[Chem.Mol]:
        ...
        # Ensure smiles are strings, not numpy arrays
        smiles_list = [str(s) if not isinstance(s, str) else s for s in self.smiles]
        mol = []
        for s in smiles_list:
            if s in SMILES_TO_MOL:
                ...
                if m is None:
                    ...
                    if m is None:
                        # If still None, raise an error with the problematic SMILES
                        raise ValueError(f"Invalid SMILES string: '{s}' (type: {type(s).__name__})")
                mol.append(m)

        if cache_mol():
            for s, m in zip(smiles_list, mol):
                SMILES_TO_MOL[s] = m
...
class MoleculeSampler(Sampler):
    ...
    def __init__(
        ...
        """
        # PyTorch 2.1+ removed the data_source argument from Sampler.__init__.
        # Call the no-arg base initializer for compatibility across versions.
        super().__init__()
