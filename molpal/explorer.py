    ...
    **kwargs
    ...
    def completed(self) -> bool:
        ...
        if self.iter > self.max_iters or len(self) >= self.budget or self.exhausted_pool:
            return True
        stop_state = self._plateau_state(self.iter)
        if not stop_state["ready"]:
            return False
        return stop_state["plateau_streak"] >= self.stop_patience
    ...
    def explore_initial(self) -> float:
        ...
        objective_time = time() - objective_start
        ...
        self.record_iteration_stats(
            iteration=self.iter,
            fit_time_sec=0.0,
            predict_time_sec=0.0,
            acquire_time_sec=0.0,
            objective_time_sec=objective_time,
        )
    ...
    def explore_batch(self) -> Optional[float]:
        ...
        inputs = self.acquirer.acquire_batch(
            ...
            self.iter - 1,
            fingerprints=self.pool.fps() if self.acquirer.metric == "md_ei" else None,
        )
        acquire_time = time() - acquire_start
    ...
    def load(self, chkpt_file: str):
        ...
        self.iteration_stats = state.get("iteration_stats", [])

        preds_npz = np.load(state["preds"])
        self.Y_mean = preds_npz["Y_mean"] if "Y_mean" in preds_npz else preds_npz["Y_pred"]
    ...
    def _plateau_state(self, completed_rounds: int) -> Dict[str, Optional[float]]:
        state = {
            "ready": False,
            "rolling_avg_prev_window": None,
            "relative_improvement": None,
            "plateau_hit": False,
            "plateau_streak": 0,
        }
        if completed_rounds < self.stop_min_iterations:
            return state
        if len(self.recent_avgs) <= self.window_size:
            return state

        plateau_streak = 0
        for idx in range(self.window_size, len(self.recent_avgs)):
            current = self.recent_avgs[idx]
            previous = self.recent_avgs[idx - self.window_size : idx]
            rolling_avg = sum(previous) / len(previous)
            improvement = None
            plateau_hit = False
            if rolling_avg != 0:
                improvement = (current - rolling_avg) / abs(rolling_avg)
                plateau_hit = improvement <= self.delta
            plateau_streak = plateau_streak + 1 if plateau_hit else 0
            if idx == len(self.recent_avgs) - 1:
                state.update(
                    {
                        "ready": True,
                        "rolling_avg_prev_window": rolling_avg,
    ...
    def record_iteration_stats(
        self,
        iteration: int,
        fit_time_sec: float,
        predict_time_sec: float,
        acquire_time_sec: float,
        objective_time_sec: float,
    ) -> None:
        valid_new = [score for score in self.new_scores.values() if score is not None]
        stop_state = self._plateau_state(iteration + 1)
        row = {
            "iteration": iteration,
            "explored_n": len(self),
            "new_batch_n": len(self.new_scores),
            "new_valid_n": len(valid_new),
            "top_k_avg_internal": self.top_k_avg,
            "best_score_internal": self.best_score(),
            "rolling_avg_prev_window": stop_state["rolling_avg_prev_window"],
            "relative_improvement": stop_state["relative_improvement"],
            "plateau_hit": stop_state["plateau_hit"],
            "plateau_streak": stop_state["plateau_streak"],
            "should_stop_internal": stop_state["plateau_streak"] >= self.stop_patience,
            "fit_time_sec": fit_time_sec,
            "predict_time_sec": predict_time_sec,
            "acquire_time_sec": acquire_time_sec,
            "objective_time_sec": objective_time_sec,
        }

        self.iteration_stats = [
            old_row for old_row in self.iteration_stats if old_row.get("iteration") != iteration
        ]
        self.iteration_stats.append(row)
        self.iteration_stats.sort(key=lambda item: item["iteration"])
        self.write_iteration_stats()

    def write_iteration_stats(self) -> None:
        metrics_csv = self.path / "iteration_metrics.csv"
        fieldnames = [
            "iteration",
            "explored_n",
            "new_batch_n",
            ...
            "top_k_avg_internal",
            "best_score_internal",
            "rolling_avg_prev_window",
            "relative_improvement",
            "plateau_hit",
            "plateau_streak",
            "should_stop_internal",
            "fit_time_sec",
            "predict_time_sec",
            "acquire_time_sec",
            "objective_time_sec",
        ...
        with open(metrics_csv, "w", newline="") as fid:
            writer = csv.DictWriter(fid, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(self.iteration_stats)
